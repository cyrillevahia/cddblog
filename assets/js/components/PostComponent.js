...
const PostComponent = () => {
    ...
    const [rub, setRub] = useState([rubrique]);

    // Mettre à jour la rubrique à chaque nouveau changement de page
    ...
    const tabRubrique = rub.map(liste => {
        return (
            <div>{liste.rubrique}</div>
        )
    ...
    const renderList = posts.map((tabpost) => {
        const { id, theme, titre, chapo, post, image, createdAt } = tabpost;
            ...
            <div>
                <article className="entry" key={id}>
                    ...
                    <h2 className="entry-title">
                        <Link to={`/post/${id}`}>{theme} : {titre}</Link>
                    </h2>
